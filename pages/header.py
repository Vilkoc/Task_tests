...
class Header(Methods):
    ...
    def click_icon(self):
        ...
        self.click_element(self.locators_header.ICON)

    def check_dropdown(self):
        flag = self.get_attr_value(self.locators_header.CHECK_DROPDOWN, 'aria-expanded')
        if flag == 'true':
            return True
        else:
            return False
    ...
    def select_option(self, pick_item):
        ...
        if self.check_dropdown():
            self.click_element_by_text(self.locators_header.DROPDOWN, pick_item)
        else:
            self.click_icon()
    ...
    def person_verify(self, person):
        ...
        while default_person < person_criteria[person]:
            default_person = len(self.get_elements(self.locators_header.NAVBAR))
        if len(self.get_elements(self.locators_header.NAVBAR)) == person_criteria[person]:
            return True
        return False
    ...
    def go_to_allVacancies(self):
        ...
        self.click_element_double_locator(self.locators_your_resume.SHOW_ALL_INFO_BUTTON,
